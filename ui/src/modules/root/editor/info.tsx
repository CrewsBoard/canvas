...
export default function InfoEditor() {
  ...
  const [description, setDescription] = useState('This is the start node of the LLM chain.');
  ...
  return (
    <div className="max-w-lg mx-auto p-4 space-y-4 bg-white rounded-lg shadow-md">
      ...
      <p className="text-xs text-gray-500">Global nodes are available to all agents.</p>

      <div>
        <Label htmlFor="node-name">Node Name</Label>
        <Input id="node-name" value={nodeName} onChange={e => setNodeName(e.target.value)} />
        <p className="text-xs text-gray-500">Should be a short, descriptive name for the node.</p>
      </div>
      ...
      <Card>
        <CardContent className="p-3">
          ...
          <p className="text-xs text-gray-500 mt-1">
            (Required if this node is global) Defines what the node does and is important when
            routing from and to this node.
          </p>
        </CardContent>
      </Card>
      ...
      <Card>
        <CardContent className="p-3">
          ...
          />
          <p className="text-xs text-gray-500 mt-1">
            The final instructions given to the AI when the routing agent chooses this node.
          </p>
        </CardContent>
      </Card>
